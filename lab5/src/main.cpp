...
void build_counts(int n, int nprocs, std::vector<int>& row_counts,
                  std::vector<int>& row_displs, std::vector<int>& elem_counts,
                  std::vector<int>& elem_displs) {
    row_counts.assign(nprocs, 0);
    row_displs.assign(nprocs, 0);
    elem_counts.assign(nprocs, 0);
    elem_displs.assign(nprocs, 0);
    ...
    for (int p = 0; p < nprocs; p++) {
        ...
        elem_counts[p] = row_counts[p] * n;
        elem_displs[p] = row_displs[p] * n;
    }
}
...
int main(int argc, char** argv) {
    ...
    std::vector<int> row_counts, row_displs, elem_counts, elem_displs;
    build_counts(n, nprocs, row_counts, row_displs, elem_counts, elem_displs);
    ...
    if (rank == 0) {
        double sec = t1 - t0;
        long long flops = 2LL * n * n * n - static_cast<long long>(n) * n;
